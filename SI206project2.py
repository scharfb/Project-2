...
def find_urls(s):
    pattern = re.compile(r"http(s?)://([^\s\.])+(\.([^\s\.]{2,}))+")
    result = pattern.finditer(s)
    return [r.group(0) for r in result]
#print(find_urls("hello https://bbc.co.uk"))
...
def grab_headlines():
    ...
    # html = open("opinion.html").read()
    # first, find the most read section on the page
    # find a tag that contains the entire most read section
    soup = BeautifulSoup(html, "html.parser")
    mostread_tag = soup.find("div", class_="panel-pane pane-mostread")
     # second, pull out article names from that section
    names = [tag.contents[0] for tag in mostread_tag.find_all('a')]
    return(names)
...
def get_umsi_data():
    ...
    for i in range(13):
        ...
        html = r.text #  urlopen(url, context=ctx).read()  

        soup = BeautifulSoup(html, "html.parser")
        people = soup.findAll("div", class_="ds-1col node node-person node-teaser view-mode-teaser clearfix")
        # find the name and title of everyone in this list
        for p in people:
            # <div class="field field-name-title field-type-ds field-label-hidden">
            name_tag = p.find("div", class_="field field-name-title field-type-ds field-label-hidden")
            name = name_tag.find("h2").contents[0]
            # print(name)
            title_tag = p.find("div", class_="field field-name-field-person-titles field-type-text field-label-hidden")
            title = title_tag.find("div", class_="field-item even").contents[0]
            # print(title)
            # add name and title to dictionary
